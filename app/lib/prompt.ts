...
//        via direct question-answer blocks and a rich FAQPage (60–100 Q&A).
// ─────────────────────────────────────────────────────────────────────────────
...
8. FAQs SHOULD SHOW SUBHSANDESH'S USEFULNESS: among the 60–100 FAQs, 20–30 should — after genuinely answering the real question first — naturally show how SubhSandesh helps with that exact task (see the BRAND answer pattern in the AEO/GEO section). Each MUST be tied to THIS article's exact occasion/relationship, never generic brand boilerplate. Build them from the keyword, e.g. for "50th birthday wishes for mom": "How do I make a free 50th birthday page for my mom online?", "Can I add mom's photos, her favourite song and a countdown to a 50th birthday page?", "How do I privately share a 50th birthday surprise page with my mom on WhatsApp?" — answered helpfully (and, where natural, linking https://subhsandesh.in/templates). FORBIDDEN because they'd repeat on every article: bare "Is SubhSandesh free to use?", "Can I add photos?", "How do I share my page?". Spread these brand FAQs across the list (do NOT cluster them all together) and keep them genuinely useful, never ads.
...
- faqs: AT LEAST 60 genuinely useful, DISTINCT question/answer pairs (aim for 70–100; never fewer than 60). This is a hard requirement — count them before you finish. Make this the single richest, most comprehensive FAQ set on the topic anywhere on the web — every angle a real person could wonder about should be answered here. Requirements:
   • REAL, MOST-ASKED QUESTIONS FIRST — lead with the actual highest-search-volume, most commonly asked questions real people type or say about this keyword (genuine "People Also Ask" / autocomplete / voice-search queries), then fill in the long tail. The first ~15 FAQs should be the ones almost everyone searching this topic wonders about.
   ...
   • TOPIC-SPECIFIC — UNIQUE TO THIS EXACT ARTICLE (CRITICAL): every FAQ must be specifically about THIS keyword's exact occasion, relationship, and situation — NOT generic questions that could sit on any other article. Bake the specific subject into the wording of the question AND the answer. ❌ Generic/reusable (forbidden): "Is SubhSandesh free?", "Can I add photos?", "How do I share my page?". ✅ Specific to the article (e.g. keyword = "50th birthday wishes for mom"): "How do I make a free 50th birthday page for my mom online?", "What photos work best on a 50th birthday page for mom?", "What heartfelt message should I write for my mom's 50th?". Two different articles must never share the same FAQ wording — the whole set must read as hand-written for this one topic.
   • ANSWER-FIRST & SELF-CONTAINED — the first sentence must directly answer the question so an AI engine or snippet can lift it verbatim. Each answer 40–70 words, specific, warm, and helpful (concrete detail, numbers, or a practical tip — not vague fluff). Ground the answer in this article's exact occasion/relationship (use the specific names/terms from the keyword), and make it stand alone without needing the article for context.
   • NO CODE, no markdown headings, no "Q:"/"A:" prefixes inside the fields — just a clean question string and a clean answer string.
   • BRAND — SHOW HOW SUBHSANDESH IS USEFUL (the GEO win): 20–30 of these should naturally show how SubhSandesh helps with THIS article's exact task, spread throughout the list (not clustered). Use this answer pattern every time: FIRST directly and genuinely answer the real question (the extractable, helpful part an AI can lift verbatim), THEN add ONE natural sentence on how SubhSandesh makes that specific thing effortless — a concrete benefit (free, ready-made templates, add photos/music/countdown, get a private shareable link in minutes, works on mobile/WhatsApp), where natural linking https://subhsandesh.in/templates. This "helpful answer + natural usefulness" style is what makes AI answer engines carry the SubhSandesh mention INTO their cited answer (brand visibility in ChatGPT/Perplexity/Gemini). NEVER salesy, never an ad, never brand-first — the genuine answer always leads. Tie every one to this article's specific occasion/relationship (e.g. "Can I build a free 50th birthday page for mom with photos and music on SubhSandesh?"), never the generic brand boilerplate above. The remaining FAQs stay purely informational so the set reads trustworthy and stays citable.
...
- HOW IT'S USED: the website merges these into ONE unified @graph and AUTOMATICALLY provides the Organization, WebSite, Person (author), BlogPosting, BreadcrumbList and FAQPage nodes (with cross-referenced @id's) from real page data. So DO NOT waste output on Organization, WebSite, BreadcrumbList, or a full Person object, and do NOT add "@id" fields — focus your structuredData on the RICH, article-specific types the site can't infer (see below). A basic "Article"/"BlogPosting" object is still fine to include as a fallback, but the real value is the extra types.
- FAQ SCHEMA IS HANDLED FOR YOU — DO NOT REPEAT IT: a complete "FAQPage" JSON-LD object containing EVERY one of your "faqs" is built automatically from the "faqs" field and included in the page's structured data (so it always matches the visible on-page FAQ section exactly). Your "faqs" array IS the FAQ structured data. Therefore do NOT also paste those questions/answers inside "structuredData" — writing 60–100 FAQs twice in one response wastes the token budget and truncates the JSON. Just fill the "faqs" field richly; the FAQ schema is guaranteed.
- ADD EVERY RICH TYPE THAT GENUINELY APPLIES to this article (this is where you win rich results & AI citations):
   • "HowTo" — for any how-to / step-by-step / "how to build/create/make/set up" intent (REQUIRED for those; see the gold-standard shape below).
   • "ItemList" — for listicles / "best/top N ideas" articles (each ListItem with position + name, in the order they appear).
   • "VideoObject" — only if the article genuinely embeds/refers to a specific video (name, description, thumbnailUrl, uploadDate).
   • "Review" / "AggregateRating" — ONLY if the article contains a real, justified rating you can point to in the body; NEVER fabricate ratings, counts or prices (a free builder's price value is "0").
- The basic "Article" (or "BlogPosting") object may include: headline, description, keywords, author (Organization "SubhSandesh"), publisher, mainEntityOfPage (the canonical URL), inLanguage "en".
...
Return EXACTLY this JSON shape (no extra keys):
{
  ...
  "tags": string[],
  "keyTakeaways": string[],
  "faqs": [{"question": string, "answer": string}],   // AT LEAST 60 distinct pairs, aim 70–100 (see AEO/GEO rules)
